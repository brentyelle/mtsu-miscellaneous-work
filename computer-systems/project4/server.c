...
int main(int argc, char**argv) {
    ...
    while (1) {
        ...
        clientptr = malloc(sizeof(ClientInfo));
        ...
        pthread_create(&tid, NULL, thread, clientptr);
        
        //then repeat the thread, waiting for a new client
    }
    return 0;
}
...
void PrintOptions(int cfd) {
    ...
    strcpy(buf, "Welcome to the Student Records server.\tPlease select from the following choices:\t");
//    strcat(buf, " 1 : Add a student's information to the records.\t");
    strcat(buf, " 1 : Search for a student's information within the records.\t");
    strcat(buf, " 2 : Close the connection.\t");
...
void RePrintOptions(int cfd) {
    ...
    //message to send to client, all concatenated together
    strcpy(buf, "You must chose a number 1 or 2:\t");
//    strcat(buf, " 1 : Add a student's information to the records.\t");
    strcat(buf, " 1 : Search for a student's information within the records.\t");
    strcat(buf, " 2 : Close the connection.\t");
    strcat(buf, "Enter selection: \n");
